...
import pandas as pd  #functions for analyzing, cleaning, exploring, and manipulating data.
...
for i in range(0,len(l1)):
    l2.append(0)
    df=pd.read_csv (r'C:\College Study Marterial\5th Semester\Training.csv.xls')
...
#print(X)                                             
y = df[["prognosis"]]
np.ravel  #The numpy module of Python provides a function called numpy. ravel, which is used to change a
          # 2-dimensional array or a multi-dimensional array into a contiguous flattened array. 
          #The returned array has the same data type as the source array or input array.
#print ok
#Read a csv named testing.csv
tr=pd.read_csv (r'C:\College Study Marterial\5th Semester\Testing.csv.xls')
...
'(vertigo) Paroymsal  Positional Vertigo':36,'Acne':37,'Urinary tract infection':38,'Psoriasis':39,
'Impetigo':40}},inplace=True)
X_test= tr[l1] #symptoms
y_test = tr[["prognosis"]] #diseases
#print(y_test)
np.ravel(y_test)

def DecisionTree():
    from sklearn import tree
    clf3 = tree.DecisionTreeClassifier()  #used for performing multiclass classification on dataset.
    clf3 = clf3.fit(X,y) #method will build a decision tree classifier from given training set (X, y). #training
    from sklearn.metrics import accuracy_score
    y_pred=clf3.predict(X_test) #predicting the disease on new test data x_test
    print(accuracy_score(y_test, y_pred)) #this function computes subset accuracy between prdicted and true disease
    print(accuracy_score(y_test, y_pred,normalize=False)) #If noarmlise=False, return the number of correctly classified samples
...
def randomforest():
    ...
    clf4 = clf4.fit(X,np.ravel(y)) #trainng
...
#lables
w2 = Label(root, justify=LEFT, text="Disease Prediction using Machine Learning", fg="Red", bg="White")
w2.config(font=("Times",30,"bold italic"))
w2.grid(row=1, column=0, columnspan=2, padx=200)
w2 = Label(root, justify=LEFT, text="Made By Muskan Agarwal "+"CSE "+"5 Semester", fg="Pink", bg="Blue")
w2.config(font=("Times",30,"bold italic"))
w2.grid(row=2, column=0, columnspan=2, padx=200)
